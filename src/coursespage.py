...
def display_course_info(row):
    ...
    with col1:
        ...
        st.write(f"💰 Price: ₹{int(row['price']):,}", help="Total price in Indian Rupees")
        st.write("⏱️ Duration: ", row["duration_weeks"])
        location = row["location"]
        if str(location).lower() == "online":
            st.write(f"📍 Location: Online")
        else:
    ...
    with col2:
        ...
        st.write(f"Start Date: " + row['start_date'].strftime("%d-%b-%Y"))
        st.write(f"End Date: " + row['end_date'].strftime("%d-%b-%Y"))
        st.write(f"Duration: ", row["duration_weeks"])
